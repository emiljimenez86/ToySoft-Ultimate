...
function mostrarModalProducto(codigo = null, soloComponente = false) {
    ...
    if (codigo) {
        ...
        if (productoSeleccionado) {
            ...
            // Cargar productos principales si es componente (casillas)
            if (tipo === 'componente') {
                cargarProductosPOSEnSelector();
                marcarProductosComponente(getProductosPrincipalesDeComponente(productoSeleccionado));
            }
...
function toggleCamposComponente() {
    ...
    if (tipoProducto === 'componente') {
        ...
        marcarProductosComponente([]);
    }
}

function obtenerProductosSeleccionadosComponente() {
    return Array.from(document.querySelectorAll('#listaProductosComponente input[type="checkbox"]:checked'))
        .map(cb => cb.value)
        .filter(Boolean);
}

function actualizarResumenProductosComponente() {
    const resumen = document.getElementById('resumenProductosComponente');
    if (!resumen) return;
    const n = obtenerProductosSeleccionadosComponente().length;
    if (n === 0) {
        resumen.textContent = 'Marque todos los platos que usan este insumo. El stock es uno solo y se descuenta al vender cualquiera.';
    } else if (n === 1) {
        resumen.textContent = '1 plato seleccionado. El stock se descuenta al vender ese plato.';
    } else {
        resumen.textContent = `${n} platos seleccionados. Un solo stock; se descuenta al vender cualquiera de ellos.`;
    }
}

function marcarProductosComponente(nombres) {
    const set = new Set((nombres || []).map(n => (n || '').toLowerCase()));
    document.querySelectorAll('#listaProductosComponente input[type="checkbox"]').forEach(cb => {
        cb.checked = set.has((cb.value || '').toLowerCase());
    });
    actualizarResumenProductosComponente();
}

function marcarTodosProductosComponente(marcar) {
    document.querySelectorAll('#listaProductosComponente .form-check:not([style*="display: none"]) input[type="checkbox"]').forEach(cb => {
        cb.checked = !!marcar;
    });
    if (marcar) cargarCategoriaDesdeProducto();
    actualizarResumenProductosComponente();
}

function filtrarListaProductosComponente() {
    const q = (document.getElementById('filtroProductosComponente')?.value || '').trim().toLowerCase();
    const lista = document.getElementById('listaProductosComponente');
    if (!lista) return;

    lista.querySelectorAll('.form-check').forEach(row => {
        const nombre = (row.dataset.nombre || '').toLowerCase();
        row.style.display = !q || nombre.includes(q) ? '' : 'none';
    });

    lista.querySelectorAll('.grupo-categoria').forEach(grupo => {
        let el = grupo.nextElementSibling;
        let hayVisible = false;
        while (el && !el.classList.contains('grupo-categoria')) {
            if (el.classList.contains('form-check') && el.style.display !== 'none') hayVisible = true;
...
function cargarProductosPOSEnSelector() {
    ...
    const seleccionPrev = obtenerProductosSeleccionadosComponente();
    const filtro = document.getElementById('filtroProductosComponente');
    if (filtro) filtro.value = '';

    lista.innerHTML = '';

    if (!productosPOS.length) {
        lista.innerHTML = '<p class="text-muted small mb-0">No hay productos en el POS. Créelos en Administración.</p>';
        actualizarResumenProductosComponente();
        return;
    }
    ...
    Object.keys(productosPorCategoria).sort().forEach(categoria => {
        ...
        titulo.className = 'grupo-categoria';
        ...
        productosPorCategoria[categoria]
            ...
            .forEach(producto => {
                ...
                wrap.dataset.nombre = producto.nombre || '';

                const cb = document.createElement('input');
                cb.type = 'checkbox';
                cb.className = 'form-check-input';
                cb.value = producto.nombre || '';
                cb.id = `plato-comp-${lista.childElementCount}`;
                cb.addEventListener('change', () => {
                    cargarCategoriaDesdeProducto();
                    actualizarResumenProductosComponente();
                });
    ...
    if (seleccionPrev.length) marcarProductosComponente(seleccionPrev);
    else actualizarResumenProductosComponente();
}

function cargarCategoriaDesdeProducto() {
    const primerSeleccionado = obtenerProductosSeleccionadosComponente()[0];
...
function guardarProducto() {
    const form = document.getElementById('formProducto');
    const tipoProducto = document.getElementById('tipoProducto').value;
    const productosSeleccionados = obtenerProductosSeleccionadosComponente();
    ...
    if (tipoProducto === 'componente' && productosSeleccionados.length === 0) {
        alert('Marque al menos un plato del POS que lleve este componente.');
        const filtro = document.getElementById('filtroProductosComponente');
                                            ...
                                            <div class="card bg-dark border-warning h-100">
                                                <div class="card-body">
                                                    ...
                                                    <ol class="small mb-0 ps-3">
                                                        ...
                                                        <li>Elige los <strong>platos del POS</strong> que llevan este componente (marca con casilla: Calentado, Ejecutivo, etc.).</li>
                            ...
                            <div class="col-12">
                                ...
                                <div class="alert alert-primary bg-dark border-primary">
                                    <ul class="mb-0">
                                        ...
                                        <li><strong>Flujo de Componentes:</strong> Usa el botón "+ Ingrediente / Insumo" para crear componentes y marcar los platos del POS que los llevan. Un mismo componente puede usarse en varios productos.</li>
